...
public class SSLHelper {
  ...
  public SSLHelper(HttpClientOptions options, KeyStoreHelper keyStoreHelper,
  ...
  public SSLHelper(HttpServerOptions options, KeyStoreHelper keyStoreHelper,
                   KeyStoreHelper trustStoreHelper) {
    this.ssl = options.isSsl();
    this.keyStoreHelper = keyStoreHelper;
    this.trustStoreHelper = trustStoreHelper;
    ...
    this.crlValues =
        options.getCrlValues() != null ? new ArrayList<>(options.getCrlValues()) : null;
    this.enabledCipherSuites = options.getEnabledCipherSuites();
  }

  public SSLHelper(NetClientOptions options, KeyStoreHelper keyStoreHelper,
                   KeyStoreHelper trustStoreHelper) {
    this.ssl = options.isSsl();
    this.keyStoreHelper = keyStoreHelper;
    this.trustStoreHelper = trustStoreHelper;
   ...
   */
  private static TrustManager[] createUntrustRevokedCertTrustManager(TrustManager[] trustMgrs,
                                                                     ArrayList<CRL> crls) {
    ...
    for (int i = 0; i < trustMgrs.length; i++) {
      ...
      if (trustMgr instanceof X509TrustManager) {
        ...
        trustMgrs[i] = new X509TrustManager() {
          ...
          private void checkRevocaked(X509Certificate[] x509Certificates)
              throws CertificateException {
  ...
  public SslHandler createSslHandler(VertxInternal vertx, boolean client, String host, int port) {
    final SSLEngine engine = getContext(vertx).newEngine(ByteBufAllocator.DEFAULT, host, port);
    return createSslHandler(engine, client);
  }


  public SslHandler createSslHandler(VertxInternal vertx, boolean client) {
    final SSLEngine engine = getContext(vertx).newEngine(ByteBufAllocator.DEFAULT);
    return createSslHandler(engine, client);
  }

  private SslHandler createSslHandler(final SSLEngine engine, final boolean client) {
    if (enabledCipherSuites != null && !enabledCipherSuites.isEmpty()) {
      String[] toUse = enabledCipherSuites.toArray(new String[enabledCipherSuites.size()]);
      engine.setEnabledCipherSuites(toUse);
    }
